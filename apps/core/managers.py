...
class UserManager(BaseManager):
    def create_user(self, phone_number, social_code=None, password=None, **kwargs):
        if not phone_number:
        ...
        user = self.model(phone_number=phone_number, social_code=social_code, **kwargs)

        user.set_password(password if password else phone_number)

        user.save(using=self._db)
        logger.info(f"User created successfully: {phone_number}")
        return user

    def create_superuser(self, phone_number, social_code, password=None, **kwargs):
