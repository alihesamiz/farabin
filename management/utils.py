...
def check_file_ready(company_name: str):
    ...
    if not latest_file:
        logger.error(
            f"File for company {company_name} not found or not ready.")
    ...
    logger.info(
        f"File for company {company_name} is ready for use: {latest_file}")
    return latest_file
...
def get_updated_file_path(company_name: str):
    ...
    new_file_path = os.path.join(
        folder, f"updated_{company_name}_{timestamp}.xlsx")
    logger.info(
