...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        match action:
            case 'backup':
                self.job()
                logger.info(
                    "Backup scheduler started. Waiting for next run...")
                self.stdout.write(self.style.SUCCESS(
                    "Successfully created and uploaded a backup."))
            case "restore":
                self.restore()
                logger.info("Backup downloaded and restored successfully.")
                self.stdout.write(self.style.SUCCESS(
                    "Successfully downloaded and restored a backup."))
            case _:
                logger.error(
                    "Invalid action. Please use 'backup' or 'restore'.")
    ...
    def get_latest_backup(self, drive_service):
        ...
        try:
            ...
            if not files:
                raise Exception(
                    "No backup files found in Google Drive folder.")
    ...
    def restore(self):
        ...
        try:
            ...
            self.download_backup(
                drive_service, latest_file['id'], local_backup_file)
